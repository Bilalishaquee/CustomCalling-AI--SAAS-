...
const BlockPalette: React.FC = () => {
  ...
  return (
    <div className="w-64 bg-gray-900 border-l border-gray-800 p-4">
      <div className="mb-4">
        <h3 className="text-lg font-semibold text-white mb-2">Block Palette</h3>
        <p className="text-sm text-gray-400">Drag blocks to canvas or click to add</p>
      </div>
        ...
        {blockTypes.map((blockType) => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center space-x-3 p-3 rounded-lg text-white transition-colors ${blockType.color}`}
            >
              <Icon className="w-5 h-5" />
              <div className="text-left flex-1">
                <div className="text-sm font-medium">{blockType.label}</div>
                <div className="text-xs opacity-80">{blockType.description}</div>
              </div>
